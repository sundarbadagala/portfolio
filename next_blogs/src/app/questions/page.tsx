...
// Rich styling and metadata for category cards
const categoryMeta: Record<string, { icon: string; bg: string; text: string; desc: string }> = {
  javascript: { icon: "🟨", bg: "bg-amber-100 dark:bg-amber-900/30", text: "text-amber-600 dark:text-amber-400", desc: "Core language syntax, closures, event loop, scopes, prototypes, and async patterns." },
  "react": { icon: "⚛️", bg: "bg-sky-100 dark:bg-sky-900/30", text: "text-sky-600 dark:text-sky-400", desc: "Hooks, reconciliation, state management, virtualization, and component architecture." },
  "Next.js": { icon: "▲", bg: "bg-neutral-100 dark:bg-neutral-800", text: "text-neutral-800 dark:text-neutral-200", desc: "Server components, App Router, SSR/SSG/ISR, dynamic routing, and optimizations." },
  "Node.js": { icon: "🟢", bg: "bg-emerald-100 dark:bg-emerald-900/30", text: "text-emerald-600 dark:text-emerald-400", desc: "Libuv event loop, event emitters, streams, cluster configurations, and APIs." },
  TypeScript: { icon: "🔷", bg: "bg-blue-100 dark:bg-blue-900/30", text: "text-blue-600 dark:text-blue-400", desc: "Static typing system, interfaces, advanced generics, utility types, and configurations." },
  MongoDB: { icon: "🍃", bg: "bg-green-100 dark:bg-green-900/30", text: "text-green-600 dark:text-green-400", desc: "NoSQL document storage, indexing, aggregation pipelines, and data models." }
};
...
export default function Questions() {
  ...
  const filteredCategories = categories.filter(cat =>
    ...
    (categoryMeta[cat] && categoryMeta[cat].desc.toLowerCase().includes(searchQuery.toLowerCase()))
  );
  ...
  return (
    <main className="min-h-screen py-10 bg-neutral-50/50 dark:bg-neutral-950/20">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center !justify-start gap-4 pb-6 border-b border-neutral-150 dark:border-neutral-800">
                          <div className="flex items-center gap-4 text-left">
                            ...
                            <div>
                              ...
                              {/* <p className="text-xs text-neutral-400 mt-1 leading-relaxed">{meta.desc}</p> */}
                            </div>
                          </div>
                        ...
                        <AnimatePresence>
                          {showSubCategoriesContent && (
                            <motion.div
                              ...
                              className="mt-8"
                            >
                              {/* Inner Header Title / Search: Only shown when no subcategory is expanded */}
                              {/* Remove Search Subtopics */}
                                      ...
                                      >
                                        {isSubSelected ? (
                                          <div className="animate-slide-up w-full">
                                            ...
                                            {/* Difficulty level filter & search row inside the expanded card! */}
                                            <AnimatePresence>
                                              {showQuestionsContent && (
                                                <motion.div
                                                  ...
                                                  className="mt-6"
                                                >
                                                  <DifficultyFilter handleSelectDifficulty={(value: any) => {
                                                    setDifficultyFilter(value as "all" | "beginner" | "medium" | "high");
                                                  }}
